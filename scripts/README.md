# scripts/ — MetaClaw Raspberry Pi Deployment Guide

> **Acerca de MetaClaw**: MetaClaw es un fork personal de [ZeroClaw](https://github.com/zeroclaw-labs/zeroclaw).

Este directorio contiene todo lo necesario para compilar cruzadamente MetaClaw y desplegarlo en una Raspberry Pi sobre SSH.
...
| `rpi-config.toml` | Production config template deployed to `~/.metaclaw/config.toml` |
| `metaclaw.service` | systemd unit file installed on the Pi |
...
| `RPI_DIR` | `~/metaclaw` | Remote directory for the binary and `.env` |
...
2. **Stop service** — runs `sudo systemctl stop metaclaw` on the Pi (continues if not yet installed).
3. **Create remote directory** — ensures `$RPI_DIR` exists on the Pi.
4. **Copy binary** — SCPs the compiled binary to `$RPI_DIR/metaclaw`.
...
6. **Deploy config** — copies `rpi-config.toml` to `~/.metaclaw/config.toml`, preserving any `api_key` already present in the file.
7. **Install systemd service** — copies `metaclaw.service` to `/etc/systemd/system/`, then enables and restarts it.
...
## Interacting with MetaClaw on the Pi
...
```bash
ssh pi@raspberrypi.local 'journalctl -u metaclaw -f'
```
...
MetaClaw is deployed with the `peripheral-rpi` feature, which enables two LLM-callable tools:
...
|------------|--------|-------------|
| `~/metaclaw/metaclaw` | compiled binary | Main agent binary |
| `~/metaclaw/.env` | created on first deploy | API key and environment variables |
| `~/.metaclaw/config.toml` | `rpi-config.toml` | Agent configuration |
| `/etc/systemd/system/metaclaw.service` | `metaclaw.service` | systemd service unit |
...
To customise, edit `~/.metaclaw/config.toml` directly on the Pi and restart the service.
...
```bash
ssh pi@raspberrypi.local 'sudo systemctl status metaclaw'
ssh pi@raspberrypi.local 'journalctl -u metaclaw -n 50 --no-pager'
```
...
```bash
ssh pi@raspberrypi.local 'file ~/metaclaw/metaclaw'
